'use strict';

module.exports = {

  handleGetIgnored: function handleGetIgnored(data, client) {
    client.getIgnored(function (ignored) {
      client.sendXt('gn', -1, ignored);
    });
  },

  handleAddIgnore: function handleAddIgnore(data, client) {
    var target = parseInt(data[3]);

    if (client.buddies.includes(target)) return;
    if (client.ignored.includes(target)) return;

    client.addIgnore(target);
    client.sendXt('gn', -1, target);
  },

  handleRemoveIgnore: function handleRemoveIgnore(data, client) {
    var target = parseInt(data[3]);

    if (client.ignored.includes(target)) {
      client.removeIgnore(target);
      client.sendXt('rn', -1, target);
    }
  }

};